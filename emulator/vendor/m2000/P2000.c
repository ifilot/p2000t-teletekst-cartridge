...
/* Added for the P2000T Teletekst cartridge emulator (2026): SLOT2 Pico W. */
extern void PicoCard_Out(byte port, byte value);
extern byte PicoCard_In(byte port);
extern void EmulatorControl_Out(byte port, byte value);
extern byte EmulatorControl_In(byte port);
...
{
 int i;
 if (Port >= 0xfc) { EmulatorControl_Out(Port, Value); return; }
 switch (Port>>4)
 {
...
{
 if (Port >= 0xfc) return EmulatorControl_In(Port);
 switch (Port>>4)
 {
