...
int main(int argc,char **argv){const char *monitor=NULL,*cart=NULL,*font="emulator/assets/Default.fnt",*fixture=NULL;int live=0,frames=0;
  ...
  struct page_backend backend={fixture,live};p2wp_device_init(page_backend_fetch,&backend);if(protocol_version)p2wp_device_set_protocol_range((uint8_t)protocol_version,(uint8_t)protocol_version);if(status_length)p2wp_device_set_status_length((uint8_t)status_length);OutputReg|=0x40;
  ...
  int ok=dump_screen()&&dump_frame();TrashP2000();SDL_Quit();return ok?0:1;
}
