...
static const char *dump_path,*dump_frame_path,*dump_fetches_path,*dump_pages_path;
...
static void automatic_keyboard(int frame){static int stage,pressed=-1,release_frame,legacy_warning_seen,next_action_frame;
  ...
  else if(stage==3&&frame>=150&&screen_has("KIES BRON (0-3)")){code=auto_source==3?4:auto_source==2?63:auto_source==1?46:45;stage=auto_source==0?4:5;}
...
int main(int argc,char **argv){const char *monitor=NULL,*cart=NULL,*font="emulator/assets/Default.fnt",*fixture=NULL;int live=0,frames=0;
  for(int i=1;i<argc;i++){if(!strcmp(argv[i],"--monitor")&&++i<argc)monitor=argv[i];else if(!strcmp(argv[i],"--cartridge")&&++i<argc)cart=argv[i];
    ...
    else if(!strcmp(argv[i],"--dump-pages")&&++i<argc)dump_pages_path=argv[i];else{fprintf(stderr,"bad argument: %s\n",argv[i]);return 2;}}
  if(!monitor||!cart||auto_source<0||auto_source>3||(auto_source==0&&auto_keys&&!auto_custom_server&&!flash_path)||protocol_version<0||protocol_version>255||(status_length!=0&&status_length!=5&&status_length!=9&&status_length!=13&&status_length!=17)){fprintf(stderr,"Usage: p2000t-emulator --monitor ROM --cartridge ROM [--live|--fixture JSON] [--auto-source 0|1|2|3 --custom-server URL] [--flash FILE] [--p2wp-version N] [--p2wp-status-length 5|9|13|17]\n");return 2;}
  ...
  int ok=dump_screen()&&dump_frame()&&dump_fetches(&backend)&&dump_pages(&backend);TrashP2000();SDL_Quit();return ok?0:1;
}
