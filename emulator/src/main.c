...
static const char *dump_path,*dump_frame_path,*dump_fetches_path,*dump_pages_path,*dump_sources_path;
...
byte EmulatorControl_In(byte port){if(port==0xfd)return queued_head!=queued_tail?(queued_keys[queued_head%sizeof(queued_keys)]==0x58?3:1):0;if(port==0xfe&&queued_head!=queued_tail)return queued_keys[queued_head++%sizeof(queued_keys)];return 0;}
...
static int keycode(SDL_Keycode key){
  ...
  switch(key){case SDLK_SPACE:return 17;case SDLK_RETURN:return 52;case SDLK_BACKSPACE:return 44;
    ...
    case SDLK_LSHIFT:return 72;case SDLK_RSHIFT:return 79;case SDLK_KP_ENTER:return 16;
    case SDLK_KP_0:return 19;case SDLK_KP_1:return 59;case SDLK_KP_2:return 58;
    case SDLK_KP_3:return 56;case SDLK_KP_4:return 67;case SDLK_KP_5:return 66;
    case SDLK_KP_6:return 64;case SDLK_KP_7:return 51;case SDLK_KP_8:return 50;
    case SDLK_KP_9:return 48;default:return -1;}
}
...
static int named_keycode(const char *name,size_t length){
  static const struct {const char *name;int code;} keys[]={
    {"START",128},{"LEFT",0},{"RIGHT",23},{"BACKSPACE",44},{"STOP",0x58},
    {"KP0",19},{"KP1",59},{"KP2",58},{"KP3",56},{"KP4",67},
    {"KP5",66},{"KP6",64},{"KP7",51},{"KP8",50},{"KP9",48}
  };
  for(size_t i=0;i<sizeof(keys)/sizeof(keys[0]);i++)
    if(strlen(keys[i].name)==length&&!memcmp(name,keys[i].name,length))return keys[i].code;
  return length==1?ascii_keycode(name[0]):-1;
}
static int next_action_key(size_t *position){
  while(auto_action[*position]==',')(*position)++;
  if(!auto_action[*position])return -1;
  size_t start=*position;
  while(auto_action[*position]&&auto_action[*position]!=',')(*position)++;
  return named_keycode(auto_action+start,*position-start);
}
static void automatic_keyboard(int frame){static int stage,pressed=-1,release_frame,legacy_warning_seen,next_action_frame,source_cycles_done,source_action_frame=150;
  ...
  int code=-1;
  if(stage==0&&auto_wait_opening&&screen_has("Emulated WiFi"))stage=1;
  else if(stage==0&&!auto_wait_opening&&frame>=5){code=17;stage++;}
    ...
    if(wifi_password[wifi_password_position])code=ascii_keycode(wifi_password[wifi_password_position++]);
    else code=52;}
  else if(stage==2&&screen_has("WIFI-PROFIEL BEWAREN")){code=25;stage++;}
  else if(stage==3&&frame>=source_action_frame&&screen_has("KIES BRON")){
    if(source_cycles_done<auto_source_cycles){code=34;source_cycles_done++;source_action_frame=frame+12;}
    else{code=auto_source==3?4:auto_source==2?63:auto_source==1?46:45;stage=auto_source==0?4:5;}}
  else if(stage==4&&pressed<0&&auto_source==0&&screen_has("SERVERADRES")){
    if(auto_custom_server&&custom_position==0&&RAM&&RAM[0x18c0])code=44;
    else if(auto_custom_server&&auto_custom_server[custom_position])code=ascii_keycode(auto_custom_server[custom_position++]);
    else{code=52;stage=5;}}
  else if(stage==5&&auto_action&&RAM&&RAM[0x18c7]&&frame>=next_action_frame){ /* cartridge page-valid byte at 0x78c7 */
    code=next_action_key(&action_position);next_action_frame=frame+12;
    if(code<0)stage=6;}
...
static int dump_pages(const struct page_backend *backend){
  if(!dump_pages_path)return 1;
  FILE *f=fopen(dump_pages_path,"wb");if(!f)return 0;
  size_t count=backend->request_count<sizeof(backend->requested_pages)/sizeof(backend->requested_pages[0])?backend->request_count:sizeof(backend->requested_pages)/sizeof(backend->requested_pages[0]);
  for(size_t i=0;i<count;i++)if(fprintf(f,"%u\n",backend->requested_pages[i])<0){fclose(f);return 0;}
  return fclose(f)==0;
}
static int dump_sources(const struct page_backend *backend){
  if(!dump_sources_path)return 1;
  FILE *f=fopen(dump_sources_path,"wb");if(!f)return 0;
  size_t count=backend->request_count<sizeof(backend->requested_sources)?backend->request_count:sizeof(backend->requested_sources);
  int ok=fwrite(backend->requested_sources,1,count,f)==count;return fclose(f)==0&&ok;
}
...
int main(int argc,char **argv){const char *monitor=NULL,*cart=NULL,*font="emulator/assets/Default.fnt",*fixture=NULL;int live=0,frames=0;
  for(int i=1;i<argc;i++){if(!strcmp(argv[i],"--monitor")&&++i<argc)monitor=argv[i];else if(!strcmp(argv[i],"--cartridge")&&++i<argc)cart=argv[i];
    ...
    else if(!strcmp(argv[i],"--auto-source-cycles")&&++i<argc)auto_source_cycles=atoi(argv[i]);
    else if(!strcmp(argv[i],"--custom-server")&&++i<argc)auto_custom_server=argv[i];
    else if(!strcmp(argv[i],"--flash")&&++i<argc)flash_path=argv[i];
    else if(!strcmp(argv[i],"--auto-key")&&++i<argc)auto_action=argv[i];
    else if(!strcmp(argv[i],"--auto-keys")&&++i<argc)auto_action=argv[i];
    else if(!strcmp(argv[i],"--auto-wait-opening"))auto_wait_opening=1;
    ...
    else if(!strcmp(argv[i],"--wifi-profile"))wifi_profile=1;
    else if(!strcmp(argv[i],"--wifi-security")&&++i<argc)wifi_security=atoi(argv[i]);
    ...
    else if(!strcmp(argv[i],"--dump-fetches")&&++i<argc)dump_fetches_path=argv[i];
    else if(!strcmp(argv[i],"--dump-pages")&&++i<argc)dump_pages_path=argv[i];
    else if(!strcmp(argv[i],"--dump-sources")&&++i<argc)dump_sources_path=argv[i];
    else if(!strcmp(argv[i],"--fail-page")&&++i<argc)fail_page=atoi(argv[i]);
    else if(!strcmp(argv[i],"--fail-error")&&++i<argc)fail_error=atoi(argv[i]);
    else{fprintf(stderr,"bad argument: %s\n",argv[i]);return 2;}}
  if(!monitor||!cart||auto_source<0||auto_source>3||auto_source_cycles<0||(auto_source==0&&auto_keys&&!auto_custom_server&&!flash_path)||protocol_version<0||protocol_version>255||wifi_security<0||wifi_security>1||(status_length!=0&&status_length!=5&&status_length!=9&&status_length!=13&&status_length!=17&&status_length!=21)||fail_page<0||fail_page>899||fail_error<0||fail_error>255||((fail_page==0)!=(fail_error==0))){fprintf(stderr,"Usage: p2000t-emulator --monitor ROM --cartridge ROM [--live|--fixture JSON] [--auto-source 0|1|2|3 --custom-server URL] [--flash FILE] [--p2wp-version N] [--p2wp-status-length 5|9|13|17|21] [--wifi-profile] [--wifi-security 0|1] [--fail-page PAGE --fail-error CODE]\n");return 2;}
  ...
  struct page_backend backend={.fixture=fixture,.live=live,.fail_page=(uint16_t)fail_page,.fail_error=(uint8_t)fail_error};p2wp_device_init(page_backend_fetch,&backend);p2wp_device_set_flash_path(flash_path);if(protocol_version)p2wp_device_set_protocol_range((uint8_t)protocol_version,(uint8_t)protocol_version);if(status_length)p2wp_device_set_status_length((uint8_t)status_length);p2wp_device_set_profile_present(wifi_profile);p2wp_device_set_wifi_security((uint8_t)wifi_security);OutputReg|=0x40;
  ...
  int ok=dump_screen()&&dump_frame()&&dump_fetches(&backend)&&dump_pages(&backend)&&dump_sources(&backend);TrashP2000();SDL_Quit();return ok?0:1;
}
